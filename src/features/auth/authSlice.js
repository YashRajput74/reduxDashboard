...
const authSlice = createSlice({//create a slice
    ...
    reducers: {//functions to tell redux how to change state
        login: (state, action) => {//a reducer function to tell store what is changing 'state' ad gives payload optionally with it, also the type is generally store in this way: 'auth/login' inside action object: action: { type: 'auth/login', payload: { name: "John" } }
